...
function be_display_posts_shortcode($atts)
{
    ...
    while ($dps_listing->have_posts()) :
        ...
        if ((isset($keywords_array[0]) and strlen($keywords_array[0]) < 1) or count($keywords_array) > 1) { // PHP < 8 stuff
            // Variable to count the number of keywords found in $link_text
            $keywords_found = 0;

            // Iterate through each keyword to check its presence in $link_text
        ...
        if ($category_display && is_object_in_taxonomy(get_post_type(), $category_display)) {
            ...
            if (!empty($terms) && !is_wp_error($terms)) {
                ...
                $category_display_text = '<br><span class="category-display"><span class="category-display-label">' . $category_label . '</span> ' . implode(', ', $term_output) . '</span>';
            }
...
function be_dps_survey_admin_notice()
{
    ...
    <div class="notice notice-info is-dismissible display-posts-survey-notice">
        ...
        <p>
            <strong><?php echo wp_kses(__('~ Bill Erickson<br>Developer of Display Posts', 'display-posts'), array('br' => array())); ?></strong>
        </p>
        <p>
            <a href="https://displayposts.com/user-survey?utm_source=displaypostsplugin&utm_medium=link&utm_campaign=survey_notice"
               class="display-posts-dismiss-survey-notice display-posts-survey-out" target="_blank"
               rel="noopener"><?php esc_html_e('Yes, I will!', 'display-posts'); ?></a><br>
            <a href="#" class="display-posts-dismiss-survey-notice" target="_blank"
               rel="noopener noreferrer"><?php esc_html_e('Nope, maybe later', 'display-posts'); ?></a><br>
            <a href="#" class="display-posts-dismiss-survey-notice" target="_blank"
               rel="noopener noreferrer"><?php esc_html_e('I already did', 'display-posts'); ?></a>
        </p>
    </div>
